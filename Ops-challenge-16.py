...
def check_password():
    ...
    try:
        ...
        if user_string in word_list:
            print(f"The string '{user_string}' appeared in the word list.")
        else:
            print(f"The string '{user_string}' did not appear in the word list.")
    except FileNotFoundError:
        print(f"File not found at path: {filepath}")  # Print an error message if the file is not found
    except Exception as e:
...
""")
        if mode == "1":
            ...
            check_password()  # This calls the check_password function for Mode 2
        elif mode == '3':
